...
test('状态1: 全文预览——工具栏元信息/查找框常显/禁用导航', async ({ page }) => {
  ...
  // 查找框常显（2026-08-19 应用户要求撤销收纳）：输入框默认可见、无收纳图标入口
  await expect(page.locator('[data-testid="text-find-input"]')).toBeVisible()
  await expect(page.locator('[data-testid="text-find-entry"]')).toHaveCount(0)
  // 无查询：无计数、▲▼ 禁用；⌘F 仅聚焦全选
...
test('状态2: 搜索 co(string) → 3 匹配 + 上下文 + 省略块 + A/B 切换 + Esc 恢复', async ({ page }) => {
  ...
  // ⌘F 聚焦常显查找框，输入表达式查询（即时过滤，200ms 防抖）
  await page.keyboard.press('Meta+f')
