<template>
  ...
  <div class="fixed inset-0 z-[80] flex items-center justify-center bg-black/45" @click.self="emit('close')">
    <form class="w-[600px] max-h-[80vh] flex flex-col rounded-xl border border-border bg-bg-secondary p-5 shadow-2xl" @submit.prevent="confirm">
      ...
      <p v-if="conflicts.length > 0" class="mt-2 text-xs text-red-400">{{ $t('dialogs.batchRename.conflictError', conflicts.length) }}</p>

      <div class="mt-5 flex justify-end gap-2">
        <button type="button" class="rounded px-3 py-2 text-sm text-text-secondary hover:bg-bg-hover" @click="emit('close')">{{ $t('dialogs.batchRename.cancel') }}</button>
        <button type="submit" class="rounded bg-accent-blue px-3 py-2 text-sm text-white disabled:opacity-40" :disabled="!canSubmit">{{ $t('dialogs.batchRename.submit', files.length) }}</button>
      </div>
    </form>
  </div>
</template>
