<template>
  ...
  >
    ...
    >
      <!-- Tab icon：文件浏览 tab 用文件夹，预览 tab 用眼睛 -->
      <svg v-if="tab.preview" class="w-3.5 h-3.5 flex-shrink-0" :class="tab.id === tabsStore.activeTabId ? 'text-accent-blue' : 'text-text-secondary'" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
      </svg>
      <svg v-else class="w-3.5 h-3.5 flex-shrink-0" :class="tab.id === tabsStore.activeTabId ? 'text-accent-blue' : 'text-text-secondary'" fill="none" stroke="currentColor" viewBox="0 0 24 24">
