<template>
  <div
    class="finder-search-control relative flex items-center h-8 bg-bg-secondary/50 border rounded-lg transition-all focus-within:bg-bg-secondary"
    :class="[invalid ? 'is-invalid' : '', 'border-border/50 focus-within:border-accent-blue/50']"
  >
    <span v-if="slots.leading" class="pl-2.5 pr-0.5 flex items-center flex-shrink-0 text-text-tertiary">
      <slot name="leading" />
    </span>
    <input
      ref="inputRef"
 ...
 *
 * 视觉与 FilePane 搜索输入一致：h-8、bg-bg-secondary/50、border-border/50、
 * rounded-lg；聚焦 border-accent-blue/50 + 实底。宽度由调用方类控制
 * （FilePane：w-36 focus-within:w-52；文本预览：w-[280px] 固定不伸缩）。
 *
